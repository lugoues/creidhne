...
// checkUniqueServices guards against two units generating the same systemd
// service name (e.g. a network "foo" and a container "foo-network" both yield
// foo-network.service). Distinct filenames make checkUniqueFilenames miss it,
// but it silently corrupts service->unit resolution (svcToID) and is a real
// conflict on the host.
func checkUniqueServices(quads []eval.Quadlet) error {
	owner := map[string]string{}
	for _, q := range quads {
		for _, u := range q.Units {
			if u.Service == "" {
