...
// nestedBool reads a bool under data[section][key], reporting whether it was
// present (so an absent optional field is distinguishable from an explicit false).
func nestedBool(data map[string]any, section, key string) (val, present bool) {
	sec, ok := data[section].(map[string]any)
	if !ok {
		return false, false
	}
